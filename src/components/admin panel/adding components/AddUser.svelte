<script>
	...
	async function handleClick() {
		...
		if (response) {
			addNotification(`Žák ${name} úspěšně přidán`, 'success');
		} else {
			addNotification(`Žák ${name} již existuje`, 'wrong');
		}
...
<div class="grid-container">
	...
	<div class="item-2 item">
		<input bind:value={name} placeholder="Jméno">
	</div>

	<div class="item-3 item">
		<input bind:value={id} placeholder="Isic">
	</div>
	<div class="item-4 item">
		<input bind:value={grade} placeholder="Třída">
	</div>
...
<style lang="scss">
  .grid-container {
    ...
    .item-5 {
      ...
      button:hover {
        background-color: rgba(70, 191, 127, 0.66);
      }
    }
  }
